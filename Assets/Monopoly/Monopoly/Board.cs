...
namespace MONOPOLY
{
    public class Board
    {
        ...
        public static int[] property = new int[40] { -1, 0, -1, 1, -1, -1, 2, -1, 2, 3, -1, 4, -1, 4, 5, -1, 6, -1, 6, 7, -1, 8, -1, 8, 9, -1, 10, 10, -1, 11, -1, 12, 12, -1, 13, -1, -1, 14, -1, 15 };
        ...
        public EOutcome Step(int d1, int d2)
        {
            switch (mode)
            {
                case EMode.ROLL: return Roll(d1, d2);
            }

            return EOutcome.ONGOING;
        }

        public EOutcome Roll(int d1, int d2)
        {
            BeforeTurn();
        ...
        public void ActivateTile()
        {
            ...
            if (tile == ETile.PROPERTY)
            {
                ...
                if (owner == BANK_INDEX)
                {
                    Player.EDecision decision = players[turn].DecideBuy(index);
                ...
                {
                    int house_price = BUILD[property[index]];
                    

                    if (players[turn].funds >= house_price && houses[index] < 5)
                    {
                        Player.EDecision decision = players[turn].DecideBuildHouse(index);

                        if (decision == Player.EDecision.YES)
                        {
                            Payment(turn, house_price);
                            players[turn].Purchase(index);
